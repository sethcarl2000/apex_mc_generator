	subroutine mc_hrsl (p_spec, dpp, x, y, z, dxdz, dydz,
...
C The arguments
	...
	real*8  pathlen
        real*8  x_ascii_tmp, y_ascii_tmp, xp_ascii_tmp
	real*8  yp_ascii_tmp
	...
	logical skipto_Q1_flag                  !if true, skip to Q1 2/3 (septum)
...
C This is a test to make sure that initialized variables are working. 2 Jul 25

C I'm going to hard-code some parameters here that I'm not going to use (Seth 2 Jul 25)	
	resmult = 1.000
	collimator = 0
	th_spec = 0.000
	ms_flag = .false.
...
C  bool   _wcs_flag(false);
C  bool   _decay_flag(false);
C  double _resmult(0.);
C  double _fry(0.);target_sources(test_exec PRIVATE 
    

C  bool   _ok_spec(false);
C  double _pathlen(0.);
	   write (*,*)  "collimator setting: ",collimator
C  double _x_ascii_tmp(0.);
C  double _y_ascii_tmp(0.);
C  double _xp_ascii_tmp(0.);
C  double _yp_ascii_tmp(0.); 
	endif
...
C       Check if we should skip to the Q1 (APEX setup, using septum)
...
C       If the right flag is enabled, skip to Q1 entrance. 
	if (skipto_Q1_flag) then
C       write (*,*) "Skipped to Q1."
	   xs = x
	   ys = y
	   zs = z
	   dxdzs = dxdz
