...
#include <math.h>
#define DEBUG_BFIELD_FRINGE_Q 1
...
BField_Fringe_Q::BField_Fringe_Q(G4int num, G4double pGradient1, G4double qrad1, G4ThreeVector pOrigin, G4RotationMatrix* pMatrix, G4int QuadNumber)
{
  ...
  {
     onceonly=false;
     for (float y_tmp=1000.; y_tmp > -900.; y_tmp-=10.)
     {
       float snake_xx_tmp=140.;
       float snake_yy_tmp=y_tmp;
       float snake_zz_tmp=0.1;
       float grd1_tmp = 1.;
       float rad1_tmp = 150.;
//       snake_xx_tmp=x_tmp*10.;
       float bx_tmp=0, by_tmp=0, bz_tmp=0;
       short int n_tmp=1;
/*
       mpoles1_(&n_tmp, &snake_xx_tmp, &snake_zz_tmp, &snake_yy_tmp,
              &grd1_tmp, &rad1_tmp,
              &bx_tmp, &bz_tmp, &by_tmp);
*/
       double b_new[3]={0};
       mpolescpp(n_tmp, snake_xx_tmp, snake_zz_tmp, snake_yy_tmp, grd1_tmp, rad1_tmp, b_new);
       bx_tmp=b_new[0];
       by_tmp=b_new[2];
       bz_tmp=b_new[1];

       std::cout<<"Fillllll("<<y_tmp<<",  "<<bz_tmp<<";"<<std::endl;
     }
  }

}

BField_Fringe_Q::~BField_Fringe_Q()
{
  delete fpMatrix;
}

void BField_Fringe_Q::GetFieldValue( const G4double y[7],
				 G4double B[3]  ) const  
{
  ...
  G4ThreeVector r_local = G4ThreeVector
   ...
   if (r_tmp < fRadius1)
   if ( (snake_yy < 100.*cm) && (snake_yy > 0.*m) )  //Vardan: 50 cm is enough for the fring field length - farther B~10e-4 Tesla and less
   {
...
#ifdef DEBUG_BFIELD_FRINGE_Q

#endif

}
void BField_Fringe_Q::mpolescpp(int in, double nx, double ny, double nz, double ngradn,double nrad,double nb[3]) const
{
...
void BField_Fringe_Q::bplscpp ( int igp, double d, double s, double gg[6]) const
{
      double c0=0.1039;
      double c1=6.27108;
      double c2=-1.51247;
      ...
      {
//        std::cout<<"fringe sos:"<<nQ1Sos<<std::endl;
        c0=0.2486;
        c1=5.3626;
        c2=-2.412;
        c3=0.9874;
